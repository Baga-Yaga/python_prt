...
def submit_details():
	name = e1.get()
    age = e2.get()
    gender = e3.get()  # Get selected gender
    address = e4.get()
    contactno=e5.get()
        
        # Call the insertDonor function to add donor details to the database
    insertDonor(name, age, gender, address, contactno)
    root.destroy()  # Close the donor details window after submitting

    # Submit button
b1 = Button(root, text="Submit", command=submit_details).place(x=40, y=300)
